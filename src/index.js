...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (!user.pro && numTodos >= 10) {
    return response.status(403).json({ error: "Your plan only allows create 10 todos"});
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!todo) {
    return response.status(404).json({ error: " Todo not created "})
  };
